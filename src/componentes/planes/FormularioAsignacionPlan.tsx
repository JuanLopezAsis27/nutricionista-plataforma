...
interface Props {
  planId: string;
  /** Paciente prefijado (asignación desde la ficha). */
  pacienteIdFijo?: string;
  onTerminado: () => void;
}

/** Formulario para asignar un plan a un paciente. */
export function FormularioAsignacionPlan({ planId, pacienteIdFijo, onTerminado }: Props) {
  const { asignar, delPaciente } = usePlanes();

  const form = useForm<DatosFormulario>({
    resolver: zodResolver(esquema),
    defaultValues: { pacienteId: pacienteIdFijo ?? "", fechaInicio: hoyISO(), fechaFin: "" },
  });

  const pacienteId = form.watch("pacienteId");
  const planActivo = delPaciente({ pacienteId }, { enabled: Boolean(pacienteId) });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(alEnviar)} className="space-y-4">
        ...
        {pacienteId && planActivo.data && planActivo.data.id !== planId && (
          <div className="flex items-start gap-2 rounded-md border border-yellow-300/60 bg-yellow-50 p-3 text-sm text-yellow-800 dark:border-yellow-500/30 dark:bg-yellow-500/10 dark:text-yellow-200">
            ...
            <span>
              Este paciente ya tiene un plan activo («{planActivo.data.nombre}»). Al asignar
              este plan, el anterior se desactivará.
            </span>
          </div>
        )}
