...
describe("EliminarPlantilla", () => {
  it("elimina una plantilla no-sistema", async () => {
    ...
    const repo = mockPlantillaEmailRepositorio({
      obtenerPorId: vi.fn(async () =>
        plantillaEmailEjemplo({ deSistema: false }, "pla-2"),
      ),
      eliminar,
    });
  ...
  it("rechaza eliminar una plantilla de sistema", async () => {
    ...
    const repo = mockPlantillaEmailRepositorio({
      obtenerPorId: vi.fn(async () =>
        plantillaEmailEjemplo({ deSistema: true }),
      ),
      eliminar,
    });

    await expect(
      new EliminarPlantilla(repo).ejecutar("pla-1"),
  ...
  it("lanza ErrorPlantillaNoEncontrada si no existe", async () => {
    ...
    await expect(
      new EliminarPlantilla(repo).ejecutar("x"),
