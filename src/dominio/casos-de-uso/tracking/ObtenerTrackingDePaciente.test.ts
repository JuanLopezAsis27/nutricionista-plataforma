...
describe("ObtenerTrackingDePaciente", () => {
  ...
  it("calcula la concordancia por franja contra el plan activo", async () => {
    const plan = planEjemplo({
      comidas: [
        { nombre: "Desayuno", horaDesde: "08:00", opciones: [{ contenido: "café" }] },
        { nombre: "Almuerzo", horaDesde: "13:00", opciones: [{ contenido: "pollo" }] },
      ],
    });
    ...
    const desayuno = t.concordancia.porFranja.find((f) => f.franja === "Desayuno")!;
  ...
  it("arma la serie de peso combinando diario y antropometría", async () => {
    const t = await tracking({
      ...
      mediciones: [antropometriaEjemplo({ fecha: new Date("2026-07-01"), pesoKg: 80 })],
    });
