...
describe("ImportarAlimentos", () => {
  it("reemplaza la lista con las filas válidas y descarta las vacías", async () => {
    ...
    const uc = new ImportarAlimentos(mockAlimentoPropioRepositorio({ reemplazarTodos }));
    ...
    expect(enviados.map((x) => x.aPrimitivos().nombre)).toEqual(["Arroz", "Pollo"]);
  });

  it("lanza si no hay ninguna fila válida", async () => {
    const uc = new ImportarAlimentos(mockAlimentoPropioRepositorio());
    await expect(uc.ejecutar([{ nombre: "" }])).rejects.toBeInstanceOf(ErrorValidacion);
  });
...
describe("ObtenerEstadoAlimentosPropios", () => {
  ...
  it("activo=false cuando la lista está vacía", async () => {
    const uc = new ObtenerEstadoAlimentosPropios(mockAlimentoPropioRepositorio());
...
describe("VaciarAlimentosPropios", () => {
  it("vacía la lista del inquilino", async () => {
    ...
    const uc = new VaciarAlimentosPropios(mockAlimentoPropioRepositorio({ vaciar }));
    await uc.ejecutar();
