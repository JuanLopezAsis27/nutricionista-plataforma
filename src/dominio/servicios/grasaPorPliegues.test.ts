...
describe("catálogo de métodos", () => {
  ...
  it("Jackson & Pollock y Parrillo ya no están entre los métodos vigentes", () => {
    expect(METODOS_GRASA).not.toContain("JACKSON_POLLOCK_7");
    expect(METODOS_GRASA).not.toContain("JACKSON_POLLOCK_4");
    expect(METODOS_GRASA).not.toContain("PARRILLO");
  });
});

describe("esMetodoGrasaVigente", () => {
  it("reconoce los métodos activos", () => {
    expect(esMetodoGrasaVigente("FAULKNER")).toBe(true);
  });

  it("rechaza un método retirado de la aplicación", () => {
    expect(esMetodoGrasaVigente("PARRILLO")).toBe(false);
    expect(esMetodoGrasaVigente("JACKSON_POLLOCK_7")).toBe(false);
    expect(esMetodoGrasaVigente("JACKSON_POLLOCK_4")).toBe(false);
  });

  it("rechaza null y valores desconocidos", () => {
    expect(esMetodoGrasaVigente(null)).toBe(false);
    expect(esMetodoGrasaVigente("ALGO_INVENTADO")).toBe(false);
  });
});
